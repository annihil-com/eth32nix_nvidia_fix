...
void CHook::InitWeaponList(void)
{
	...
	case MOD_JAYMOD2:
		eth32.cg.weaponList = normalWeapons;
		eth32.cg.numWeapons = 53;
		break;
	case MOD_NOQUARTER:
		eth32.cg.weaponList = noquarterWeapons;
		eth32.cg.numWeapons = 63;
		break;
	default:
...
int CDECL eth32_UI_syscall(int command, ...)
{
	...
	switch (command)
	{
		...
		{
			//printf("UI_R_DRAW: %i %i\n", arg[8], eth32.cg.media.cursor);
			if (64 == (qhandle_t) arg[8])
			{
...
char *eth32_Cvar_VariableString(const char *var_name)
{
	...
	/*
	if (strcmp(var_name, "r_glDriver") && strcmp(var_name, "cl_profile") && strcmp(var_name, "gamestate") && strcmp(var_name, "cg_thirdperson") && strcmp(var_name, "r_shownormals") && strcmp(var_name, "r_singleshader"))
		printf("cvar: %s\n", var_name);

	if (eth32.cg.pbserver) {
		for (int i=0; i<sizeof(defaultCvarList)/sizeof(defaultCvarList[0]); i++){
			if (!strcasecmp(var_name, defaultCvarList[i].name)) {
				strcpy(buffer, defaultCvarList[i].restoreValue);
				return buffer;
			}
			buffer[0] = '0';
			buffer[1] = '\0';
